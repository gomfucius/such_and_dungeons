...
func parseCSV(){
    ...
    func element(_ string: String) -> String {
        ...
        let element = string.replacingOccurrences(of: "enumElement", with: "")
    ...
    for line in lines {
        ...
        if line != "" {
            ...
            print("monster = Monster(name: \"\(item.0)\", image: \"\(item.1)\", level: \(item.2), hpMax: \(item.3), mpMax: \(item.4), hp: \(item.3), mp: \(item.5), attackPower: \(item.5), armorClass: \(item.6), strength: \(item.7), stamina: \(item.8), intelligence: \(item.9), agility: \(item.10), xp: \(item.11), gold: \(item.12), startFloor: \(item.13), endFloor: \(item.14), availability: \(item.15), attribute: \(attribute), width: \(item.18), height: \(item.19), offset: CGPoint(x: \(numbers.first!), y: \(numbers[1])), weak: [\(element(item.21))], strong: [\(element(item.22))], drop: \"\(item.23)\", dropPercentage: \(item.24), sleepPercentage: \(item.25), skills: [\(skills)])")
